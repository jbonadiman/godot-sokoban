...
func _unhandled_key_input(event: InputEvent) -> void:
	if event.is_pressed():
		...
		var is_wall := walls_blocks.has(target_block)
		var is_closed_door: bool = \
				target_block == Types.blocks.door and \
...
func crate_at_position(at_position: Vector2) -> GameCrate:
	for child in get_parent().get_children():
		if child is GameCrate and child.position.x == at_position.x and child.position.y == at_position.y:
			return child
	return null


func block_at_position(at_position: Vector2) -> int:
	var i := 0
	...
	for y in range(-half - remainder, half):
		for x in range(-half - remainder, half):
			if at_position.x == x * 64 and at_position.y == y * 64:
